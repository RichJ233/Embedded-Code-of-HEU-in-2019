...
uint8_t *p_header,count; 
uint16_t data_length;

void unpack_fifo_handle(void)
{
	p_header = judgement_receive.buf;
	count = 0;
	while(count <= 50)
	{
		if(*p_header == SOF)
		{
			...
			p_header ++;
		count = p_header - judgement_receive.buf;
	}
}
...
void judgement_data_decode(void)
{
	switch(judgement_receive.cmd){
		case GAME_DATA:{
			memcpy(&judgement_race_data.game_robot_data, judgement_receive.data, data_length);
		};break;  
		case ROBOT_STATE_DATA:{
			memcpy(&judgement_race_data.game_state_data, judgement_receive.data, data_length);
		};break;
		case POWER_HEAT_DATA:{
			memcpy(&judgement_race_data.power_heat_data, judgement_receive.data, data_length);
		};break;  
		case ROBOT_POS_DATA:{
			memcpy(&judgement_race_data.robot_pos_data, judgement_receive.data, data_length);
		};break;
		case ROBOT_HURT_DATA:{
			memcpy(&judgement_race_data.robot_hurt_data, judgement_receive.data, data_length);
		};break;
	}
}
